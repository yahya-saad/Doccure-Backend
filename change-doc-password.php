...
if(!isset($_SESSION['doctorLogin'])){
	...
	if(isset($_POST['submit'])){
		...
		$selectPassword = "SELECT `password` FROM `doctors` WHERE id='$id'";
		$select = $conn -> query($selectPassword);
		foreach($select as $i){
			if($oldPass == $i['password'] && $nPass == $cPass) {
				$_SESSION['succmsg'] = "Password updated successfully";
				$updatePassword = "UPDATE `doctors` SET `password` ='$nPass' WHERE id = $id ";
								...
								<div class="card-body">
									<div class="row">
										<div class="col-md-12 col-lg-6">
											...
											<form action="change-doc-password.php" method="POST">
